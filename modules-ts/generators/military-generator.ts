...
Military = () => {
  ...
  const generate = function () {
    ...
    valid.forEach((s) => {
      const temp = (s.temp = {}),
        d = s.diplomacy;
      const expansionRate = Math.min(
        Math.max(s.expansionism / expn / (s.area / area), 0.25),
        4
      ...
      s.alert = Math.min(
        Math.max(rn(expansionRate * diplomacyRate * neighborsRate, 2), 0.1),
        5
      ); // war alert rate (army modifier)
      temp.platoons = [];
      ...
      for (const unit of options.military) {
        ...
        if (unit.type === 'mounted' && s.form === 'Horde') modifier *= 2;
  ...
  const drawRegiments = function (regiments, s) {
    ...
    const baseColor =
      pack.states[s].color[0] === '#' ? pack.states[s].color : '#999';
  ...
  const getEmblem = function (r) {
    ...
    if (
      ...
      cells.burg[r.cell] &&
      pack.burgs[cells.burg[r.cell]].capital
    )
