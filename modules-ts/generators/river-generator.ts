...
Rivers = () => {
  const generate = function (changeHeights = true) {
    ...
    const h = Array.from(cells.h)
      ...
      .map((h, i) =>
        h < 20 || cells.t[i] < 1
          ? h
          : h + d3.mean(cells.c[i].map((c) => cells.t[c])) / 10000
      );
      ...
      land.forEach(function (i) {
        ...
        if (cells.r[min]) {
          ...
          if (cells.fl[min] < cells.fl[i]) {
            ...
            if (h[min] >= 20)
              riversData.find((r) => r.river === cells.r[min]).parent =
                cells.r[i]; // min river is a tributary of current river
            cells.r[min] = cells.r[i]; // re-assign river if downhill part has less flux
          } else {
            cells.conf[min] += cells.fl[i]; // mark confluence
            if (h[min] >= 20)
              riversData.find((r) => r.river === cells.r[i]).parent =
                cells.r[min]; // current river is a tributary of min river
          }
    ...
    void (function defineRivers() {
      ...
      for (let r = 1; r <= riverNext; r++) {
        ...
        if (riverSegments.length > 2) {
          ...
          const source = riverSegments[0],
            mouth = riverSegments[riverSegments.length - 2];
  ...
  const addMeandring = function (segments, rndFactor = 0.3) {
    ...
    for (let s = 0; s < segments.length; s++) {
      ...
      if (P(0.5)) side *= -1; // change meandring direction in 50%
      ...
      if (dist2 > 64 || (dist2 > 16 && segments.length < 6)) {
        ...
        if (P(0.2)) side *= -1; // change 2nd extra point meandring direction in 20%
  ...
  const getPath = function (points, width = 1, increment = 1) {
    let offset,
      extraOffset = 0.1; // starting river width (to make river source visible)
    const riverLength = points.reduce(
      (s, v, i, p) =>
        s + (i ? Math.hypot(v[0] - p[i - 1][0], v[1] - p[i - 1][1]) : 0),
      0
    ); // summ of segments length
    ...
    for (let p = 1; p < last; p++) {
      ...
      offset =
        (Math.atan(Math.pow(p * factor, 2) / widening) / 2) * width +
        extraOffset;
  ...
  const specify = function () {
    ...
    Math.seedrandom(seed);
    const smallLength = pack.rivers
      .map((r) => r.length || 0)
      .sort((a, b) => a - b)[Math.ceil(pack.rivers.length * 0.15)];
  ...
  const remove = function (id) {
    ...
    const riversToRemove = pack.rivers
      .filter((r) => r.i === id || getBasin(r.i, r.parent, id) === id)
      .map((r) => r.i);
