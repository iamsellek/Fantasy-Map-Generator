class Battle {
  ...
  constructor(attacker: Army, defender: Army, customization?: Customization) {
    if (customization) {
      return;
    }

    closeDialogs('.stable');
    this.customization = 13; // enter customization to avoid unwanted dialog closing
    ...
    document
      ...
      .addEventListener('click', (ev) => this.toggleChange(ev));
    document
      .getElementById('battleType')
      .nextElementSibling.addEventListener('click', (ev) =>
        Battle.prototype.context.changeType(ev)
      );
    document
      .getElementById('battleNameShow')
      .addEventListener('click', () =>
        Battle.prototype.context.showNameSection()
      );
    document
      .getElementById('battleNamePlace')
      .addEventListener(
        'change',
        (ev) => (Battle.prototype.context.place = ev.target.value)
      );
    document
      .getElementById('battleNameFull')
      .addEventListener('change', (ev) =>
        Battle.prototype.context.changeName(ev)
      );
    document
      .getElementById('battleNameCulture')
      .addEventListener('click', () =>
        Battle.prototype.context.generateName('culture')
      );
    document
      .getElementById('battleNameRandom')
      .addEventListener('click', () =>
        Battle.prototype.context.generateName('random')
      );
    document
      .getElementById('battleNameHide')
      .addEventListener('click', this.hideNameSection);
    document
      .getElementById('battleAddRegiment')
      .addEventListener('click', this.addSide);
    document
      .getElementById('battleRoll')
      .addEventListener('click', () => Battle.prototype.context.randomize());
    document
      .getElementById('battleRun')
      .addEventListener('click', () => Battle.prototype.context.run());
    document
      .getElementById('battleApply')
      .addEventListener('click', () => Battle.prototype.context.applyResults());
    document
      .getElementById('battleCancel')
      .addEventListener('click', () =>
        Battle.prototype.context.cancelResults()
      );
    document
      .getElementById('battleWiki')
      .addEventListener('click', () => wiki('Battle-Simulator'));
    ...
    document
      ...
      .addEventListener('click', () =>
        Battle.prototype.context.rollDie('attackers')
      );
    document
      .getElementById('battleDie_defenders')
      .addEventListener('click', () =>
        Battle.prototype.context.rollDie('defenders')
      );
  }
  ...
  defineType() {
    ...
    const getType = () => {
      const typesA = Object.keys(attacker.u).map(
        (name) => options.military.find((u) => u.name === name).type
      );
      ...
      if (
        typesA.every((t) => t === 'aviation') &&
        typesD.every((t) => t === 'aviation')
      )
        return 'air'; // if attacked and defender have only aviation units
      if (attacker.n && !defender.n && typesA.some((t) => t !== 'naval'))
        return 'landing'; // if attacked is naval with non-naval units and defender is not naval
      if (!defender.n && pack.burgs[pack.cells.burg[this.cell]].walls)
        return 'siege'; // defender is in walled town
      if (P(0.1) && [5, 6, 7, 8, 9, 12].includes(pack.cells.biome[this.cell]))
        return 'ambush'; // 20% if defenders are in forest or marshes
      return 'field';
    };
  ...
  setType() {
    document.getElementById('battleType').className =
      'icon-button-' + this.type;

    const sideSpecific = document.getElementById(
      'battlePhases_' + this.type + '_attackers'
    );
    ...
    const defenders = sideSpecific
      ? document.getElementById('battlePhases_' + this.type + '_defenders')
          .content
      : attackers;

    document.getElementById(
      'battlePhase_attackers'
    ).nextElementSibling.innerHTML = '';
    document.getElementById(
      'battlePhase_defenders'
    ).nextElementSibling.innerHTML = '';
    document
      .getElementById('battlePhase_attackers')
      .nextElementSibling.append(attackers.cloneNode(true));
    document
      .getElementById('battlePhase_defenders')
      .nextElementSibling.append(defenders.cloneNode(true));
  }
  ...
  definePlace() {
    ...
    const proper =
      burg || river ? null : Names.getCulture(cells.culture[this.cell]);
  ...
  defineName() {
    ...
    if (this.type === 'air')
      return `${this.place} ${P(0.8) ? 'Air Battle' : 'Dogfight'}`;
  }
  ...
  addRegiment(side, regiment) {
    regiment.casualties = Object.keys(regiment.u).reduce(
      (a, b) => ((a[b] = 0), a),
      {}
    );
    ...
    const distance =
      (Math.hypot(this.y - regiment.by, this.x - regiment.bx) *
        distanceScaleInput.value) |
      0; // distance between regiment and its base
    ...
    let casualties = `<tr class="battleCasualties"><td></td><td data-tip="${
      state.fullName
    }">${state.fullName.slice(0, 26)}</td>`;
  ...
  addSide() {
    ...
    const distance = (reg) =>
      rn(
        Math.hypot(context.y - reg.y, context.x - reg.x) *
          distanceScaleInput.value
      ) +
      ' ' +
      distanceUnitInput.value;
    const isAdded = (reg) =>
      context.defenders.regiments.some((r) => r === reg) ||
      context.attackers.regiments.some((r) => r === reg);
    ...
    body.innerHTML = regiments
      .map((r) => {
        ...
        return `<div ${added ? "class='inactive'" : ''} data-s=${s.i} data-i=${
          r.i
        } data-state=${s.name} data-regiment=${r.name} 
        data-total=${
          r.a
        } data-distance=${dist} data-tip="Click to select regiment">
        <svg width=".9em" height=".9em" style="margin-bottom:-1px"><rect x="0" y="0" width="100%" height="100%" fill="${
          s.color
        }" class="fillRect"></svg>
  ...
  showNameSection() {
    document
      .querySelectorAll('#battleBottom > button')
  ...
  generateName(type) {
    ...
    document.getElementById(
      'battleNameFull'
    ).value = this.name = this.defineName();
    $('#battleScreen').dialog({ title: this.name });
  }
  ...
  calculateStrength(side) {
    ...
    this[side].power =
      d3.sum(
        options.military.map(
          (u) => (forces[u.name] || 0) * u.power * scheme[phase][u.type]
        )
      ) / adjuster;
  ...
  getInitialMorale() {
    const powerFee = (diff) =>
      Math.min(Math.max(100 - diff ** 1.5 * 10 + 10, 50), 100);
    const distanceFee = (dist) => Math.min(d3.mean(dist) / 50, 15);
    const powerDiff = this.defenders.power / this.attackers.power;
    this.attackers.morale =
      powerFee(powerDiff) - distanceFee(this.attackers.distances);
    this.defenders.morale =
      powerFee(1 / powerDiff) - distanceFee(this.defenders.distances);
  ...
  selectPhase() {
    ...
    const getFieldBattlePhase = () => {
      const prev = [
        this.attackers.phase || 'skirmish',
        this.defenders.phase || 'skirmish',
      ]; // previous phase
      ...
      if (prev[0] === 'skirmish' && prev[1] === 'skirmish') {
        const forces = this.getJoinedForces(
          this.attackers.regiments.concat(this.defenders.regiments)
        );
    ...
    const getNavalBattlePhase = () => {
      const prev = [
        this.attackers.phase || 'shelling',
        this.defenders.phase || 'shelling',
      ]; // previous phase
      ...
      if (!prev[0] === 'boarding') {
        if (
          powerRatio < 0.5 ||
          (P(this.attackers.casualties) && powerRatio < 1)
        )
          return ['withdrawal', 'chase'];
        if (powerRatio > 2 || (P(this.defenders.casualties) && powerRatio > 1))
          return ['chase', 'withdrawal'];
      }

      // boarding phase can start from 2nd iteration
      if (prev[0] === 'boarding' || P(i / 10 - 0.1))
    ...
    const getSiegePhase = () => {
      const prev = [
        this.attackers.phase || 'blockade',
        this.defenders.phase || 'sheltering',
      ]; // previous phase
      ...
      if (prev[0] !== 'storming') {
        const machinery = options.military
          .filter((u) => u.type === 'machinery')
          .map((u) => u.name); // machinery units
        ...
        if (
          i &&
          prev[1] !== 'sortie' &&
          machineryD < machineryA &&
          P(0.25) &&
          P(morale[1] / 70)
        )
          phase[1] = 'sortie'; // defenders sortie
      }
    ...
    const getAmbushPhase = () => {
      const prev = [
        this.attackers.phase || 'shock',
        this.defenders.phase || 'surprise',
      ]; // previous phase

      if (prev[1] === 'surprise' && P(1 - (powerRatio * i) / 5))
        return ['shock', 'surprise'];
    ...
    const getLandingPhase = () => {
      const prev = [
        this.attackers.phase || 'landing',
        this.defenders.phase || 'defense',
      ]; // previous phase
    ...
    const getAirBattlePhase = () => {
      const prev = [
        this.attackers.phase || 'maneuvering',
        this.defenders.phase || 'maneuvering',
      ]; // previous phase
      ...
      if (prev[0] === 'maneuvering' && P(1 - i / 10))
        return ['maneuvering', 'maneuvering'];
    ...
    buttonA.dataset.tip = buttonA.nextElementSibling.querySelector(
      "[data-phase='" + phase[0] + "']"
    ).dataset.tip;

    const buttonD = document.getElementById('battlePhase_defenders');
    buttonD.className = 'icon-button-' + this.defenders.phase;
    buttonD.dataset.tip = buttonD.nextElementSibling.querySelector(
      "[data-phase='" + phase[1] + "']"
    ).dataset.tip;
  }
  ...
  run() {
    ...
    const casualties =
      Math.random() *
      Math.max(
        PHASE_CASUALTY_MODIFIER[this.attackers.phase],
        PHASE_CASUALTY_MODIFIER[this.defenders.phase]
      ); // total casualties, ~10% per iteration
    ...
    // change morale
    this.attackers.morale = Math.max(
      this.attackers.morale - casualtiesA * 100 - 1,
      0
    );
    this.defenders.morale = Math.max(
      this.defenders.morale - casualtiesD * 100 - 1,
      0
    );
  ...
  updateTable(side) {
    for (const r of this[side].regiments) {
      ...
      for (const u of options.military) {
        battleCasualties.querySelector(`td:nth-child(${index})`).innerHTML =
          r.casualties[u.name] || 0;
        battleSurvivors.querySelector(`td:nth-child(${index})`).innerHTML =
          r.survivors[u.name] || 0;
        index++;
      }

      battleCasualties.querySelector(
        `td:nth-child(${index})`
      ).innerHTML = d3.sum(Object.values(r.casualties));
      battleSurvivors.querySelector(
        `td:nth-child(${index})`
      ).innerHTML = d3.sum(Object.values(r.survivors));
    }
  ...
  toggleChange(ev) {
    ...
    document
      ...
      .addEventListener('click', hideSection, { once: true });
  }
  ...
  applyResults() {
    ...
    const maxCasualties = Math.max(
      this.attackers.casualties,
      this.attackers.casualties
    );
    const relativeCasualties =
      this.defenders.casualties /
      (this.attackers.casualties + this.attackers.casualties);
    ...
    function getBattleStatus(relative, max) {
      ...
      if (relative > 95)
        return [
          'attackers flawless victory',
          'disorderly retreat of defenders',
        ];
      if (relative > 0.7)
        return ['attackers decisive victory', 'defenders disastrous defeat'];
      ...
      if (relative > 0.5)
        return ['attackers disastrous defeat', 'decisive victory of defenders'];
      if (relative >= 0)
        return [
          'attackers disorderly retreat',
          'flawless victory of defenders',
        ];
    ...
    function applyResultForSide(r, side) {
      ...
      if (note) {
        ...
        const losses = r.a
          ? Math.abs(d3.sum(Object.values(r.casualties))) / r.a
          : 1;
        const regStatus =
          losses === 1
            ? 'is destroyed'
        ...
        const casualties = Object.keys(r.casualties)
          .map((t) =>
            r.casualties[t] ? `${Math.abs(r.casualties[t])} ${t}` : null
          )
          .filter((c) => c);
        const casualtiesText = casualties.length
    ...
    void (function addMarkerSymbol() {
      if (svg.select('#defs-markers').select('#marker_battlefield').size())
        return;
      const symbol = svg
        .select('#defs-markers')
        .append('symbol')
        .attr('id', 'marker_battlefield')
    ...
    const getSide = (regs, n) =>
      regs.length > 1
        ? `${n ? 'regiments' : 'forces'} of ${list([
            ...new Set(regs.map((r) => pack.states[r.state].name)),
          ])}`
        : getAdjective(pack.states[regs[0].state].name) + ' ' + regs[0].name;
    const getLosses = (casualties) => Math.min(rn(casualties * 100), 100);

    const legend = `${this.name} took place in ${options.year} ${
      options.eraShort
    }. It was fought between ${getSide(
      this.attackers.regiments,
      1
    )} and ${getSide(this.defenders.regiments, 0)}. The ${this.type} ended in ${
      battleStatus[+P(0.7)]
    }.
      \r\nAttackers losses: ${getLosses(
        this.attackers.casualties
      )}%, defenders losses: ${getLosses(this.defenders.casualties)}%`;
  ...
  cancelResults() {
    ...
    this.attackers.regiments
      .concat(this.defenders.regiments)
      .forEach((r) => Military.moveRegiment(r, r.px, r.py));
