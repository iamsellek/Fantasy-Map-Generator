...
// remove glow if tip is acknowledged
...
function changeUIsize(value) {
  if (isNaN(+value) || +value > 4 || +value < 0.5) return;
...
function changeDialogsTransparency(value) {
  ...
  const optionButtonsColor =
    'rgba(145, 110, 127, ' + Math.min(alpha + 0.3, 1) + ')';
  ...
  document
    .querySelectorAll('.tabcontent button')
    .forEach((el) => (el.style.backgroundColor = optionButtonsColor));
  document
    .querySelectorAll('.tabcontent li')
    .forEach((el) => (el.style.backgroundColor = optionLiColor));
  document
    .querySelectorAll('button.options')
    .forEach((el) => (el.style.backgroundColor = optionLiColor));
}
...
export function applyStoredOptions() {
  ...
  if (localStorage.getItem('uiSize'))
    ...
    changeUIsize(Math.max(Math.min(rn(mapWidthInput.value / 1280, 1), 2.5), 1));
...
export function randomizeOptions() {
  ...
  if (randomize || !locked('prec'))
    precInput.value = precOutput.value = gauss(120, 20, 5, 500);
  const tMax = +temperatureEquatorOutput.max,
    tMin = +temperatureEquatorOutput.min; // temperature extremes
...
async function enter3dView(type) {
  ...
  const started = new ThreeD(canvas, type);
  if (!started) return;
