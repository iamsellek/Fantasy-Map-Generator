...
function showMapTooltip(point, e, i, g) {
  ...
  if (group === 'rivers') {
    tip(getRiverName(e.target.id) + 'Click to edit');
    return;
  }
  if (group === 'routes') {
  ...
  if (group === 'ruler') {
    if (tag === 'rect') {
      tip('Drag to split the ruler into 2 parts');
      return;
    }
    if (tag === 'circle') {
  ...
  if (group === 'lakes' && !land) {
    tip(`${capitalize(subgroup)} lake. Click to edit`);
    return;
  }
  if (group === 'coastline') {
  ...
  if (group === 'zones') {
    tip(path[path.length - 8].dataset.description);
    return;
  }
  if (group === 'ice') {
  ...
  if (layerIsOn('togglePrec') && land)
    ...
    tip('Culture: ' + pack.cultures[pack.cells.culture[i]].name);
...
function getElevation(f, h) {
  ...
  const lakeCells = Array.from(
    pack.cells.i.filter((i) => pack.cells.f[i] === f.i)
  );
  const heights = lakeCells
    .map((i) => pack.cells.c[i].map((c) => pack.cells.h[c]))
    .flat()
    .filter((h) => h > 19);
...
function getCellPopulation(i) {
  ...
  const urban = pack.cells.burg[i]
    ? pack.burgs[pack.cells.burg[i]].population *
...
document.addEventListener('keyup', (event) => {
  ...
  if (active === 'INPUT' || active === 'SELECT' || active === 'TEXTAREA')
    return; // don't trigger if user inputs a text
  if (active === 'DIV' && document.activeElement.contentEditable === 'true')
    return; // don't trigger if user inputs a text
  event.stopPropagation();
