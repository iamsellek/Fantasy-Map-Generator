...
function addRuler(x1: number, y1: number, x2: number, y2: number): void {
  const cx = rn((x1 + x2) / 2, 2);
  const cy = rn((y1 + y2) / 2, 2);
  ...
  const rulerNew = ruler
    .append('g')
    .attr('class', 'ruler')
    .call(d3.drag().on('start', dragRuler));
  rulerNew
    .append('line')
    .attr('x1', x1)
    .attr('y1', y1)
  ...
  const label =
    rn(dist * distanceScaleInput.value) + ' ' + distanceUnitInput.value;
  rulerNew
    .append('rect')
    .attr('x', cx - size * 1.5)
    .attr('y', cy - size * 1.5)
...
function dragRulerEdge() {
  ...
  if (left) line.attr('x1', x).attr('y1', y);
  else line.attr('x2', x).attr('y2', y);

  const cx = rn((x + x0) / 2, 2),
    cy = rn((y + y0) / 2, 2);
...
function rulerCenterDrag() {
  ...
  rulerNew
    .attr('transform', rulerOld.attr('transform'))
    .call(d3.drag().on('start', dragRuler));
...
function drawOpisometer() {
  ...
  const rulerNew = ruler
    ...
    .call(d3.drag().on('start', dragRuler));
  const curve = rulerNew
    .append('path')
    .attr('class', 'white')
    .attr('stroke-width', size);
  const curveGray = rulerNew
    .append('path')
    .attr('class', 'gray')
    .attr('stroke-width', size)
    .attr('stroke-dasharray', dash);
...
function drawScaleBar(): void {
  if (scaleBar.style('display') === 'none') {
    return; // no need to re-draw hidden element
  }
