...
async function getMapURL(type, subtype?) {
  ...
  if (fontStyle)
    clone.select('defs').append('style').text(fontStyle.join('\n')); // add font to style
  ...
  const svg_xml =
    `<?xml version="1.0" encoding="UTF-8" standalone="no"?>` + serialized;
  clone.remove();
...
function getFontsToLoad() {
  ...
  if (legend.node().hasChildNodes() && !webSafe.includes(legendFont))
...
function saveGeoJSON_Cells() {
  ...
  cells.i.forEach((i) => {
    ...
    cells.v[i].forEach((n) => {
      let x =
        mapCoordinates.lonW + (v.p[n][0] / graphWidth) * mapCoordinates.lonT;
      let y =
        mapCoordinates.latN - (v.p[n][1] / graphHeight) * mapCoordinates.latT; // this is inverted in QGIS otherwise
      data += '[' + x + ',' + y + '],';
    });
    // close the ring
    let x =
      mapCoordinates.lonW +
      (v.p[cells.v[i][0]][0] / graphWidth) * mapCoordinates.lonT;
    let y =
      mapCoordinates.latN -
      (v.p[cells.v[i][0]][1] / graphHeight) * mapCoordinates.latT; // this is inverted in QGIS otherwise
    data += '[' + x + ',' + y + ']';
    data += ']] },\n   "properties": {\n';
...
function saveGeoJSON_Markers() {
  ...
  markers._groups[0][0].childNodes.forEach((n) => {
    let x =
      mapCoordinates.lonW + (n.dataset.x / graphWidth) * mapCoordinates.lonT;
    let y =
      mapCoordinates.latN - (n.dataset.y / graphHeight) * mapCoordinates.latT; // this is inverted in QGIS otherwise
...
function getRiverPoints(node) {
  ...
  for (let i = l, c = i; i >= 0; i -= increment, c += increment) {
    ...
    let x =
      mapCoordinates.lonW +
      ((p1.x + p2.x) / 2 / graphWidth) * mapCoordinates.lonT;
    let y =
      mapCoordinates.latN -
      ((p1.y + p2.y) / 2 / graphHeight) * mapCoordinates.latT; // this is inverted in QGIS otherwise
    points.push([x, y]);
  }
  return points;
}
...
function parseLoadedData(data) {
  try {
    ...
    void (function parseSettings() {
      ...
      if (settings[12])
        populationRate.value = populationRateOutput.value = settings[12];
      if (settings[13])
        urbanization.value = urbanizationOutput.value = settings[13];
      if (settings[14])
        mapSizeInput.value = mapSizeOutput.value = Math.max(
          Math.min(settings[14], 100),
          1
        );
      if (settings[15])
        latitudeInput.value = latitudeOutput.value = Math.max(
          Math.min(settings[15], 100),
          0
        );
      if (settings[16])
    ...
    void (function restoreLayersState() {
      if (texture.style('display') !== 'none' && texture.select('image').size())
        turnButtonOn('toggleTexture');
      else turnButtonOff('toggleTexture');
      if (terrs.selectAll('*').size()) turnButtonOn('toggleHeight');
      else turnButtonOff('toggleHeight');
      if (biomes.selectAll('*').size()) turnButtonOn('toggleBiomes');
      else turnButtonOff('toggleBiomes');
      if (cells.selectAll('*').size()) turnButtonOn('toggleCells');
      else turnButtonOff('toggleCells');
      if (gridOverlay.selectAll('*').size()) turnButtonOn('toggleGrid');
      else turnButtonOff('toggleGrid');
      if (coordinates.selectAll('*').size()) turnButtonOn('toggleCoordinates');
      else turnButtonOff('toggleCoordinates');
      if (compass.style('display') !== 'none' && compass.select('use').size())
        turnButtonOn('toggleCompass');
      else turnButtonOff('toggleCompass');
      if (rivers.style('display') !== 'none') turnButtonOn('toggleRivers');
      else turnButtonOff('toggleRivers');
      if (terrain.style('display') !== 'none' && terrain.selectAll('*').size())
        turnButtonOn('toggleRelief');
      else turnButtonOff('toggleRelief');
      if (relig.selectAll('*').size()) turnButtonOn('toggleReligions');
      else turnButtonOff('toggleReligions');
      if (cults.selectAll('*').size()) turnButtonOn('toggleCultures');
      else turnButtonOff('toggleCultures');
      if (statesBody.selectAll('*').size()) turnButtonOn('toggleStates');
      else turnButtonOff('toggleStates');
      if (provs.selectAll('*').size()) turnButtonOn('toggleProvinces');
      else turnButtonOff('toggleProvinces');
      if (zones.selectAll('*').size() && zones.style('display') !== 'none')
        turnButtonOn('toggleZones');
      else turnButtonOff('toggleZones');
      if (borders.style('display') !== 'none') turnButtonOn('toggleBorders');
      else turnButtonOff('toggleBorders');
      if (routes.style('display') !== 'none' && routes.selectAll('path').size())
        turnButtonOn('toggleRoutes');
      else turnButtonOff('toggleRoutes');
      if (temperature.selectAll('*').size()) turnButtonOn('toggleTemp');
      else turnButtonOff('toggleTemp');
      if (prec.selectAll('circle').size()) turnButtonOn('togglePrec');
      else turnButtonOff('togglePrec');
      if (labels.style('display') !== 'none') turnButtonOn('toggleLabels');
      else turnButtonOff('toggleLabels');
      if (icons.style('display') !== 'none') turnButtonOn('toggleIcons');
      else turnButtonOff('toggleIcons');
      if (armies.selectAll('*').size() && armies.style('display') !== 'none')
        turnButtonOn('toggleMilitary');
      else turnButtonOff('toggleMilitary');
      if (markers.selectAll('*').size() && markers.style('display') !== 'none')
        turnButtonOn('toggleMarkers');
      else turnButtonOff('toggleMarkers');
      if (ruler.style('display') !== 'none') turnButtonOn('toggleRulers');
      else turnButtonOff('toggleRulers');
      if (scaleBar.style('display') !== 'none') turnButtonOn('toggleScaleBar');
      else turnButtonOff('toggleScaleBar');

      // special case for population bars
      const populationIsOn = population.selectAll('line').size();
      if (populationIsOn) drawPopulation();
      if (populationIsOn) turnButtonOn('togglePopulation');
      else turnButtonOff('togglePopulation');
    ...
    void (function restoreEvents() {
      ...
      ruler
        .selectAll('g.ruler circle')
        .call(d3.drag().on('drag', dragRulerEdge));
      ruler
        .selectAll('g.ruler circle')
        .call(d3.drag().on('drag', dragRulerEdge));
      ruler
        .selectAll('g.ruler rect')
        .call(d3.drag().on('start', rulerCenterDrag));
      ruler
        .selectAll('g.opisometer circle')
        .call(d3.drag().on('start', dragOpisometerEnd));
      ruler
        .selectAll('g.opisometer circle')
        .call(d3.drag().on('start', dragOpisometerEnd));
