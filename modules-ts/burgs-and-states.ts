declare var BurgsAndStates: () => {
  ...
  drawBurgs: any;
  specifyBurgs: any;
  defineBurgFeatures: any;
  drawStateLabels: any;
  collectStatistics: any;
  generateCampaigns: any;
  generateDiplomacy: any;
  defineStateForms: any;
  getFullName: any;
  generateProvinces: any;
  updateCultures: any;
};
...
BurgsAndStates = () => {
  ...
  const specifyBurgs = function () {
    ...
    for (const b of pack.burgs) {
      ...
      if (b.port) {
        ...
        const e = cells.v[i].filter((v) =>
          vertices.c[v].some((c) => c === cells.haven[i])
  ...
  const defineStateForms = function (list) {
    ...
    const genericArray = [],
      navalArray = []; // turn weighted array into simple array
    for (const t in generic) {
      for (let j = 0; j < generic[t]; j++) {
        genericArray.push(t);
      }
    }
    for (const t in naval) {
      for (let j = 0; j < naval[t]; j++) {
        navalArray.push(t);
      }
    }
    ...
    for (const s of states) {
      ...
      const theocracy =
        ...
        (P(0.1) && pack.religions[religion].type === 'Organized');
      s.form = theocracy
        ? 'Theocracy'
        : s.type === 'Naval'
        ? ra(navalArray)
    ...
    function selectForm(s) {
      ...
      if (s.form === 'Theocracy') {
        ...
        if (P(0.5) && [0, 1, 2, 3, 4, 6, 8, 9, 13, 15, 20].includes(base))
        ...
        if (P(0.02)) return 'Thearchy'; // "Thearchy" in very rare case
        if (P(0.05)) return 'See'; // "See" in rare case
        return 'Theocracy';
      }
    }
  ...
  const getFullName = function (s) {
    ...
    return adj.includes(s.formName)
